...
#prob([
  Let $G: {0, 1}^n -> {0, 1}^(2 n)$ be pseudorandom generator. Let $s unif {0, 1}^n, r unif {0, 1}^(2 n)$ and $y = G(s)$. Consider the following $P_(r, y)$:
  ...
  Consider the following $
    ...
    &lt.eq nu(n)
  $ since $G$ is a one-way function because its a pseudorandom generator, where $nu(n)$ is a negligible function. Note in the above we equate $cal(A) (r, a)$ and $cal(A)(r xor a, a)$. This is possible as the adversary $cal(A)$ is not actually losing information as it can recover $r$ by $(r xor a) xor a$. Therefore we have that for any PPT $cal(A)$ with input $(r, y)$ and output $x in {0, 1}^n$ we have $
    Pr[P_(r, y) (x) = 1 : x <- cal(A) (r, y)]
      &lt.eq nu(n).
  $
])
